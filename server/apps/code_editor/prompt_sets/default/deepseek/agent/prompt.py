...
<|channel|>commentary to=read_file <|constrain|>json<|message|>{"target_file":"other.py","_context_updates":[{"tc1":"file was 500 lines of CSS, irrelevant"},{"tc3":"found handleAuth at line 45"}]}

The JSON parameters must match the tool's parameter names exactly as described in the tool definitions below.

**Context compression (_context_updates):** Each tool call result is labeled with an ID like [tc1], [tc2], etc. When a tool result is no longer useful or you only need a summary, you can replace it by adding "_context_updates" to ANY subsequent tool call. Example: <|channel|>commentary to=read_file <|constrain|>json<|message|>{"target_file":"other.py","_context_updates":[{"tc1":"file was 500 lines of CSS, irrelevant"},{"tc3":"found handleAuth at line 45"}]}
Make this a regular habit on each tool call—summarize old results when they are no longer needed. Replace irrelevant results, summarize large outputs, keep only essential findings.
</tool_calling>
